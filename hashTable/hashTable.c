...
void addValueToHashTable(HashTable* hashTable, Value value, int* errorCode) {
    ...
    Position position = next(firstElement(hashTable->array[index]));
    if (isEmpty(hashTable->array[index]) || (next(position) == NULL && strcmp(getValue(hashTable->array[index], position).key, value.key))) {
        ...
        if (((float)hashTable->numberOfElementsInTheTable / (float)hashTable->size) * 10 > 6) {
            ...
            for (int i = 0; i < oldSize; ++i) {
                ...
                Position positionForAddingMemory = firstElement(tmp[i]);
                do {
        ...
        while (strcmp(getValue(hashTable->array[index], position).key, value.key) && next(position) != NULL) {
...
int getWordFrequencyFromHashTable(HashTable* hashTable, char* key) {
    ...
    Position position = firstElement(hashTable->array[index]);
    while (strcmp(getValue(hashTable->array[index], position).key, key) && next(position) != NULL) {
        position = next(position);
    }
...
void printValuesOfTheEntireHashTable(HashTable* hashTable) {
    for (int i = 0; i < hashTable->size; ++i) {
        ...
        Position position = firstElement(hashTable->array[i]);
        do {
