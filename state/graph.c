...
void findNodeInGraph(Node* node, Node** findNode, int name, List* list, int* errorCode) {
    ...
    Position positon = firstElement(node->matchList);
...
Node* getTheNearestElement(Node* node) {
    ...
    int min = 100000;
    Node* result = NULL;
    Position position = firstElement(node->matchList);
...
void findTheNearestUnoccupiedCity(Value value, int stateNumber, List* visited, List* queue, int* errorCode) {
    deleteElementWithoutErasingValues(queue, lastElement(queue));
    ...
    Position position = firstElement(value.node->matchList);
    while (next(position) != NULL) {
        ...
        if (!nodeInList(visited, valueFromQueue.node) &&
            (valueFromQueue.node->stateNumber == -1234 ||
...
Node* nodeWithMinimumLength(List* list, Node* parent) {
    ...
    while (next(position) != NULL) {
        ...
        if (tmp.length < min && tmp.node->stateNumber == -1234 &&
            parent != tmp.node) {
            min = tmp.length;
            result = tmp.node;
        }
...
void wrapFindTheNearestUnoccupiedCity(Node* node, int* errorCode) {
    ...
    while (!isEmpty(queue)) {
        Value tmp = getValue(queue, lastElement(queue));
